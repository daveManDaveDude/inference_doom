...
from tools import emit_source_stage37_selected_monster_attack_feedback_probe as stage
...
def collect_stage37_titles(pid: int, timeout_seconds: float = 8.0) -> tuple[int, list[str]]:
    deadline = time.monotonic() + timeout_seconds
    hwnd = 0
    titles: list[str] = []
    seen: set[str] = set()
    while time.monotonic() < deadline:
        ...
        if title not in seen:
            seen.add(title)
            titles.append(title)
        if "STEP37=3" in title and "S37SIG=" in title:
            return hwnd, titles
        time.sleep(0.05)
    raise TimeoutError(f"stage37 final title not reached; saw {titles!r}")


class SourceStage37SelectedMonsterAttackFeedbackProbeTests(unittest.TestCase):
    def _ref(self) -> stage.Stage37SelectedMonsterAttackFeedbackProbeReference:
        if not PINNED_WAD.exists():
            self.skipTest(f"pinned WAD missing: {PINNED_WAD}")
        return stage.reference_selected_monster_attack_feedback_probe_for_pinned_map(PINNED_WAD)

    def test_source_trace_labels_name_selected_attack_sources(self) -> None:
        labels = {entry[2] for entry in stage.SOURCE_TRACE}

        self.assertIn("A_SPosAttack_stage37_selected_shotgun_guy_feedback_debug", labels)
        self.assertIn("A_FaceTarget_stage37_selected_actor_angle_debug", labels)
        self.assertIn("P_AimLineAttack_stage37_selected_player_target_debug", labels)
        self.assertIn("P_LineAttack_stage37_selected_three_pellet_feedback_debug", labels)
        self.assertIn("P_DamageMobj_stage37_selected_player_feedback_debug", labels)
        self.assertIn("S_StartSound_stage37_selected_sfx_shotgn_boundary_debug", labels)
    ...
    def test_synthetic_a_spos_attack_target_guard_and_face_target_angle_update(self) -> None:
        ref = self._ref()
        no_target = stage.selected_attack_feedback_stage37_source_shape(ref.stage29, target_present=False)

        self.assertEqual(stage.a_spos_attack_stage37_selected(target_present=False), (0, 0, 0, 0))
    ...
    def test_synthetic_feedback_marker_and_state_signatures_include_attack_contribution(self) -> None:
        ...
        self.assertEqual(ref.signature, 2681905384)
    ...
    def test_absence_flags_keep_deferred_systems_out(self) -> None:
        ref = self._ref()
        image = stage.build_source_stage37_selected_monster_attack_feedback_probe_exe()
        lower = image.lower()
        ...
        for value in (
            ...
            ref.source_stage38_absent,
        ):
            self.assertEqual(value, 1)
        self.assertNotIn(b"source_stage38", lower)
        for forbidden in (
    ...
    def test_stage37_does_not_rely_on_full_pre_rendered_frame_arrays_for_motion(self) -> None:
        ref = self._ref()
        image = stage.build_source_stage37_selected_monster_attack_feedback_probe_exe()

        self.assertEqual(ref.full_frame_byte_arrays_absent, 1)
        self.assertNotIn(b"stage37_frame_pixels", image)
        self.assertNotIn(b"stage37_copy_rendered_frame", image)
        self.assertIn(b"NOFULL37=1", image)
    ...
    def test_executable_build_contains_stage37_markers_and_no_stage38_strings(self) -> None:
        ref = self._ref()
        image = stage.build_source_stage37_selected_monster_attack_feedback_probe_exe()

        self.assertEqual(image[:2], b"MZ")
        self.assertIn(b"source_stage37_selected_monster_attack_feedback_probe", image)
        self.assertIn(b"Selected monster attack feedback probe proof OK", image)
        self.assertIn(b"S37 ATTACK START STEP37=0", image)
        self.assertIn(b"SFX37=sfx_shotgn", image)
        self.assertIn(b"HP37=100->91", image)
        self.assertIn(b"DMG37=9", image)
        self.assertIn(f"S37SIG={ref.signature}".encode("ascii"), image)
        self.assertIn(b"S36SIG=397846180", image)
        self.assertIn(b"S19SIG=2088411722", image)
        self.assertNotIn(b"source_stage38", image.lower())

    @unittest.skipUnless(os.name == "nt", "GUI smoke test requires Windows")
    def test_smoke_launch_reports_stage37_attack_feedback_and_distinct_signatures(self) -> None:
        ref = self._ref()
        exe_path = REPO_ROOT / "build" / "source_stage37_selected_monster_attack_feedback_probe.exe"
        stage.write_source_stage37_selected_monster_attack_feedback_probe_exe(exe_path)

        process = subprocess.Popen([str(exe_path)], cwd=REPO_ROOT)
        hwnd = 0
        try:
            hwnd, titles = collect_stage37_titles(process.pid, timeout_seconds=8.0)
            joined = "\n".join(titles)
            fb_values = {
                int(match.group(1))
                for title in titles
                for match in [re.search(r"(?:^| )FB37=(\d+)", title)]
                if match
            }

            self.assertIn("S37 ATTACK START STEP37=0", joined)
            self.assertIn("STEP37=1", joined)
            self.assertIn("STEP37=2", joined)
            self.assertIn("STEP37=3", joined)
            self.assertEqual(fb_values, {sample.framebuffer_signature for sample in ref.samples})
            self.assertGreaterEqual(len(fb_values), 2)
            self.assertIn("HP37=100->91", joined)
            self.assertIn("DMG37=9", joined)
            self.assertIn("HIT37=1", joined)
            self.assertIn("MISS37=2", joined)
            self.assertIn("SFX37=sfx_shotgn", joined)
            self.assertIn("SFXC37=1", joined)
            self.assertIn("SRC37=MT_SHOTGUY->P0", joined)
            self.assertIn(f"S37SIG={ref.signature}", joined)
            self.assertIn("S36SIG=397846180", joined)
            self.assertIn("S19SIG=2088411722", joined)
            self.assertIn("S38ABS=1", joined)
        finally:
            if hwnd:
                import ctypes
