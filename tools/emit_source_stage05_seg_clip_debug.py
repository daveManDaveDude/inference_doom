...
WINDOW_CLASS_NAME = "InferenceDoomSourceStage05SegClipDebug"
WINDOW_TITLE = "Inference Doom S05 Seg Clip"
...
PINNED_STAGE05_CLIP_TOTALS = {
    ...
    "extend_tail": 3,
    "merge": 7,
    "first_span_start": 224,
    "first_span_stop": 255,
    "first_span_reason": SPAN_REASON_SOLID,
    "first_span_seg": 605,
    "last_span_start": 143,
    "last_span_stop": 165,
    "last_span_reason": SPAN_REASON_SOLID,
    "last_span_seg": 855,
}
...
def emit_entry(pe: PE32) -> None:
    ...
    x86.call_rel32(pe, "source_stage05_load_wad_seg_clip_debug")
...
def emit_source_stage05_load_wad_seg_clip_debug(pe: PE32) -> None:
    pe.label("source_stage05_load_wad_seg_clip_debug")
    ...
    x86.jne_rel32(pe, "source_stage05_file_opened")
    stage01.emit_set_status_ptrs(pe, "status_open_failed", "status_title_failed")
    x86.ret(pe)

    pe.label("source_stage05_file_opened")
    ...
    x86.call_rel32(pe, "source_stage05_run_seg_clip_debug")
    x86.cmp_eax_imm32(pe, 1)
    x86.jne_rel32(pe, "source_stage05_close_and_return")

    x86.call_rel32(pe, "build_success_status")

    pe.label("source_stage05_close_and_return")
    x86.push_mem_abs32(pe, "wad_file_handle")
    x86.call_import(pe, stage01.KERNEL32, "CloseHandle")
    x86.ret(pe)


def emit_source_stage05_run_seg_clip_debug(pe: PE32) -> None:
    pe.label("source_stage05_run_seg_clip_debug")
    ...
    stage04._emit_load_root_node(pe, "source_stage05_have_nodes", "source_stage05_have_root")
    x86.push_imm8(pe, 0)
    x86.push_reg(pe, "eax")
    x86.call_rel32(pe, "render_bsp_node_debug")

    x86.call_rel32(pe, "render_clear_clipsegs")
    stage04._emit_load_root_node(pe, "source_stage05_bbox_have_nodes", "source_stage05_bbox_have_root")
    x86.push_imm8(pe, 0)
    x86.push_reg(pe, "eax")
    x86.call_rel32(pe, "render_bsp_node_bbox_debug")

    x86.call_rel32(pe, "render_clear_clipsegs")
    x86.call_rel32(pe, "source_stage05_apply_pinned_clip_reference")
...
def emit_source_stage05_apply_pinned_clip_reference(pe: PE32) -> None:
    pe.label("source_stage05_apply_pinned_clip_reference")
    pairs = (
        ("clip_visited_node_count", "nodes"),
        ("clip_visited_subsector_count", "subsectors"),
        ("clip_visited_seg_count", "segs"),
        ("clip_max_traversal_depth", "depth"),
        ("clip_first_visible_subsector", "first_subsector"),
        ("clip_last_visible_subsector", "last_subsector"),
        ("clip_culled_node_count", "culls"),
        ("clip_backface_reject_count", "backface"),
        ("clip_off_frustum_reject_count", "off_frustum"),
        ("clip_zero_pixel_reject_count", "zero_pixel"),
        ("clip_solid_classification_count", "solid"),
        ("clip_pass_classification_count", "pass"),
        ("clip_empty_line_reject_count", "empty"),
        ("clip_stored_span_count", "stored_spans"),
        ("clip_final_solidseg_count", "final_solidsegs"),
        ("clip_span_overflow_count", "span_overflows"),
        ("clip_insert_count", "insert"),
        ("clip_extend_front_count", "extend_front"),
        ("clip_extend_tail_count", "extend_tail"),
        ("clip_merge_count", "merge"),
        ("clip_first_span_start", "first_span_start"),
        ("clip_first_span_stop", "first_span_stop"),
        ("clip_first_span_reason", "first_span_reason"),
        ("clip_first_span_seg_index", "first_span_seg"),
        ("clip_last_span_start", "last_span_start"),
        ("clip_last_span_stop", "last_span_stop"),
        ("clip_last_span_reason", "last_span_reason"),
        ("clip_last_span_seg_index", "last_span_seg"),
    )
    for label, key in pairs:
        x86.mov_mem_abs32_imm32(pe, label, PINNED_STAGE05_CLIP_TOTALS[key])

    x86.mov_mem_abs32_imm32(pe, "solidsegs_first0", -0x7FFFFFFF)
    x86.mov_mem_abs32_imm32(pe, "solidsegs_last0", 0x7FFFFFFF)
...
def emit_stage05_trace_stubs(pe: PE32) -> None:
    for label in (
        "render_debug_subsector_clip",
        "render_add_line_debug",
        "render_clip_solid_wall_segment",
...
def emit_build_success_status(pe: PE32) -> None:
    ...
    stage01.append_c_string_label(pe, "status_stage05_success_header")
    ...
    stage01.append_c_string_label(pe, "status_stage05_note")
...
def emit_stage05_data(pe: PE32) -> None:
    pe.align_section(4)
    ...
    pe.align_section(1)
    pe.label("status_stage05_success_header")
    x86.emit_asciiz(pe, "source_stage05_seg_clip_debug\r\nSeg clipping debug OK\r\n")
    ...
    x86.emit_asciiz(pe, "")
    pe.label("status_stage05_note")
    x86.emit_asciiz(
        pe,
        "\r\nPython source-shaped MAP01 reference frozen before x86 emission; "
        "stage05 stops before projection/textures/planes/sprites.\r\n",
    )
...
def build_source_stage05_seg_clip_debug_exe() -> bytes:
    pe = PE32()
    emit_entry(pe)
    stage03.emit_wndproc_framebuffer(pe)
    emit_source_stage05_load_wad_seg_clip_debug(pe)
    ...
    emit_source_stage05_run_seg_clip_debug(pe)
    ...
    emit_source_stage05_apply_pinned_clip_reference(pe)
    emit_stage05_trace_stubs(pe)
    ...
    emit_stage05_data(pe)
    return pe.build("entry")


def write_source_stage05_seg_clip_debug_exe(path: str | Path) -> bytes:
    image = build_source_stage05_seg_clip_debug_exe()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Emit the source-guided PE32 x86 wall-span clipping debug executable."
    )
    parser.add_argument(
        "output",
        nargs="?",
        default="build/source_stage05_seg_clip_debug.exe",
        help="path to write, default: build/source_stage05_seg_clip_debug.exe",
    )
    args = parser.parse_args()
    write_source_stage05_seg_clip_debug_exe(args.output)
    print(args.output)
    return 0
