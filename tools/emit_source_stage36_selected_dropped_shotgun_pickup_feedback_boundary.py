...
from tools import emit_source_stage35_selected_dropped_shotgun_visual_boundary as stage35
from tools import x86
from tools.pe32 import PE32
from tools.wad import WadFile


stage34 = stage35.stage34
stage33 = stage35.stage33
...
WINDOW_CLASS_NAME = "InferenceDoomSourceStage36SelectedDroppedShotgunPickupFeedbackBoundary"
WINDOW_TITLE = "Inference Doom S36 Dropped Shotgun Pickup Feedback Boundary"
...
STAGE36_TIMER_ID = 36
STAGE36_TIMER_MS = stage34.STAGE34_TIMER_MS
...
SOURCE_TRACE = stage35.SOURCE_TRACE + (
    ...
    (
        ...
        "R_DrawSpriteRange_stage36_selected_dropped_shotgun_world_post_table_debug",
    ),
    (
        ...
        "V_DrawBlock_stage36_selected_dropped_shotgun_visual_present_debug",
    ),
    (
        "reference/chocolate-doom/src/doom/p_inter.c",
        "P_TouchSpecialThing selected SPR_SHOT dropped-shotgun pickup branch",
        "P_TouchSpecialThing_stage36_selected_dropped_shotgun_feedback_debug",
    ),
    (
        "reference/chocolate-doom/src/doom/p_inter.c",
        "P_GiveWeapon selected wp_shotgun dropped=true one-clip semantics",
        "P_GiveWeapon_stage36_selected_dropped_shotgun_debug",
    ),
    (
        "reference/chocolate-doom/src/doom/d_items.c",
        "weaponinfo[wp_shotgun] ammo type am_shell",
        "weaponinfo_stage36_selected_shotgun_pickup_debug",
    ),
    (
        "reference/chocolate-doom/src/doom/p_mobj.c",
        "P_RemoveMobj selected dropped item removal without respawn queue",
        "P_RemoveMobj_stage36_selected_dropped_shotgun_debug",
    ),
    (
        "reference/chocolate-doom/src/doom/s_sound.c",
        "S_StartSound selected sfx_wpnup pickup boundary counter only",
        "S_StartSound_stage36_selected_wpnup_boundary_debug",
    ),
)


@dataclass(frozen=True)
class Stage36PostCommand:
    x: int
    yl: int
    yh: int
    iscale: int
...
class Stage36FrameSample:
    ...
    death_frame: int
    death_commands: tuple[Stage36PostCommand, ...]
    drop_state: int
    drop_state_name: str
    drop_sprite_name: str
    drop_patch_name: str
    drop_frame: int
    drop_commands: tuple[Stage36PostCommand, ...]
...
@dataclass(frozen=True)
class Stage36DroppedShotgunRecord:
...
@dataclass(frozen=True)
class Stage36PickupFeedback:
    special_sprite_name: str
    special_flags: int
    toucher_health: int
    special_z: int
    toucher_z: int
    ...
    delta: int
    z_reach_passed: int
    live_toucher_passed: int
    sprite_dispatch_passed: int
    dropped_weapon: int
    weapon: int
    ammo_type: int
    ammo_clip_amount: int
    ammo_before: int
    ammo_after: int
    weapon_owned_before: int
    weapon_owned_after: int
    pending_before: int
    pending_after: int
    gave_ammo: int
    gave_weapon: int
    give_weapon_return: int
    message: str
    sound: str
    sound_events: int
    bonuscount_before: int
    bonuscount_after: int
    removed_item: int
    item_present_before: int
    item_present_after: int
    respawn_queue_events: int


@dataclass(frozen=True)
class Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference:
    stage34: stage34.Stage34SelectedHitscanDeathVisualBoundaryReference
    stage17: stage17.Stage17FirstWeaponFireReference
    samples: tuple[Stage36FrameSample, ...]
    death_sources: tuple[bytes, ...]
    drop_sources: tuple[bytes, ...]
    palette32: tuple[int, ...]
    dropped_record: Stage36DroppedShotgunRecord
    pickup: Stage36PickupFeedback
    ...
    selected_item_pickup_boundary: int
    generalized_death_drop_absent: int
    selected_touch_special_boundary: int
    selected_give_weapon_boundary: int
    selected_ammo_weapon_grant_boundary: int
    selected_pickup_message_boundary: int
    selected_item_removal_boundary: int
...
def ref35_signature(ref: Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference | None = None) -> int:
    return stage35.reference_selected_dropped_shotgun_visual_boundary_for_pinned_map(WAD_PATH).signature
...
def _selected_world_commands(
    ...
    *,
    ...
    top_y: int,
    max_columns: int,
) -> tuple[tuple[Stage36PostCommand, ...], int, str, str, int, str]:
    if state_name in ("NONE", "REMOVED"):
        return (), 0, "", "", 0, state_name
...
def _draw_death_commands(
    frame: bytearray,
    commands: Sequence[Stage36PostCommand],
...
def _selected_drop_record(
    ...
    state = info.states[state_index]
    sprite_name = info.sprnames[state.sprite]
    return Stage36DroppedShotgunRecord(
...
def p_give_weapon_stage36_selected(
    *,
    owned_before: bool,
    ammo_before: int,
    max_ammo: int = stage15.MAXAMMO[stage15.AM_SHELL],
    dropped: bool = True,
) -> tuple[bool, bool, bool, int, bool, int]:
    ammo_clip_amount = stage15.CLIPAMMO[stage15.AM_SHELL] * (1 if dropped else 2)
    gave_ammo = ammo_before < max_ammo
    ammo_after = min(ammo_before + ammo_clip_amount, max_ammo) if gave_ammo else ammo_before
    gave_weapon = not owned_before
    owned_after = True if gave_weapon else owned_before
    pending_after = stage15.WP_SHOTGUN if gave_weapon else stage15.WP_PISTOL
    return gave_ammo, gave_weapon, gave_ammo or gave_weapon, ammo_after, owned_after, pending_after


def p_touch_special_thing_stage36_selected(
    *,
    special_sprite_name: str = "SHOT",
    special_flags: int,
    special_z: int = 0,
    toucher_z: int = 0,
    toucher_height: int = 56 * FRACUNIT,
    toucher_health: int = 100,
    owned_before: bool = False,
    ammo_before: int = 0,
    bonuscount_before: int = 0,
) -> Stage36PickupFeedback:
    delta = special_z - toucher_z
    z_ok = not (delta > toucher_height or delta < -8 * FRACUNIT)
    live_ok = toucher_health > 0
    sprite_ok = special_sprite_name == "SHOT"
    dropped = 1 if (special_flags & stage13.MF_DROPPED) else 0
    gave_ammo = gave_weapon = give_return = False
    ammo_after = ammo_before
    owned_after = owned_before
    pending_after = stage15.WP_PISTOL
    if z_ok and live_ok and sprite_ok:
        gave_ammo, gave_weapon, give_return, ammo_after, owned_after, pending_after = p_give_weapon_stage36_selected(
            owned_before=owned_before,
            ammo_before=ammo_before,
            dropped=bool(dropped),
        )
    removed = int(give_return)
    return Stage36PickupFeedback(
        special_sprite_name=special_sprite_name,
        special_flags=special_flags,
        toucher_health=toucher_health,
        special_z=special_z,
        toucher_z=toucher_z,
        toucher_height=toucher_height,
        delta=delta,
        z_reach_passed=int(z_ok),
        live_toucher_passed=int(live_ok),
        sprite_dispatch_passed=int(sprite_ok),
        dropped_weapon=dropped,
        weapon=stage15.WP_SHOTGUN,
        ammo_type=stage15.AM_SHELL,
        ammo_clip_amount=stage15.CLIPAMMO[stage15.AM_SHELL] * (1 if dropped else 2),
        ammo_before=ammo_before,
        ammo_after=ammo_after,
        weapon_owned_before=int(owned_before),
        weapon_owned_after=int(owned_after),
        pending_before=stage15.WP_PISTOL,
        pending_after=pending_after,
        gave_ammo=int(gave_ammo),
        gave_weapon=int(gave_weapon),
        give_weapon_return=int(give_return),
        message="GOTSHOTGUN" if give_return else "",
        sound="sfx_wpnup" if give_return else "",
        sound_events=int(give_return),
        bonuscount_before=bonuscount_before,
        bonuscount_after=bonuscount_before + stage15.BONUSADD if give_return else bonuscount_before,
        removed_item=removed,
        item_present_before=1,
        item_present_after=0 if removed else 1,
        respawn_queue_events=0,
    )


def _stage36_signature(ref: Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference) -> int:
    ...
    for value in (
        ...
        len(ref.samples),
        stage35.reference_selected_dropped_shotgun_visual_boundary_for_pinned_map(WAD_PATH).signature,
        ...
        ref.pickup.z_reach_passed,
        ref.pickup.live_toucher_passed,
        ref.pickup.sprite_dispatch_passed,
        ref.pickup.dropped_weapon,
        ref.pickup.ammo_type,
        ref.pickup.ammo_clip_amount,
        ref.pickup.ammo_before,
        ref.pickup.ammo_after,
        ref.pickup.weapon_owned_before,
        ref.pickup.weapon_owned_after,
        ref.pickup.pending_before,
        ref.pickup.pending_after,
        ref.pickup.gave_ammo,
        ref.pickup.gave_weapon,
        ref.pickup.give_weapon_return,
        ref.pickup.sound_events,
        ref.pickup.bonuscount_after,
        ref.pickup.removed_item,
        ref.pickup.item_present_before,
        ref.pickup.item_present_after,
        ref.pickup.respawn_queue_events,
...
def _reference_stage36_uncached(wad_path: str) -> Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference:
    wad = WadFile.from_file(wad_path)
    ref35 = stage35.reference_selected_dropped_shotgun_visual_boundary_for_pinned_map(wad_path)
    ...
    pickup = p_touch_special_thing_stage36_selected(
        special_flags=drop_record.final_flags,
        special_z=drop_record.final_z,
        toucher_z=drop_record.final_z,
        toucher_height=56 * FRACUNIT,
        toucher_health=100,
        owned_before=False,
        ammo_before=0,
    )
    death_sources: list[bytes] = []
    death_tables: list[tuple[Stage36PostCommand, ...]] = []
    ...
    drop_tables: list[tuple[Stage36PostCommand, ...]] = []
    drop_meta: list[tuple[int, str, str, int, str]] = []
    drop_cache: dict[str, tuple[tuple[Stage36PostCommand, ...], int, str, str, int, str]] = {}
    ...
    samples: list[Stage36FrameSample] = []
    ...
    provisional = Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference(
        stage34=ref34,
        stage17=ref17,
        ...
        selected_item_pickup_boundary=1,
        generalized_death_drop_absent=1,
        selected_touch_special_boundary=1,
        selected_give_weapon_boundary=1,
        selected_ammo_weapon_grant_boundary=1,
        selected_pickup_message_boundary=1,
        selected_item_removal_boundary=1,
        ...
        ui_systems_absent=1,
        real_audio_absent=1,
        source_stage37_absent=1,
        signature=0,
    )
    return Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference(
        **{**provisional.__dict__, "signature": _stage36_signature(provisional)}
    )


@lru_cache(maxsize=4)
def reference_selected_dropped_shotgun_pickup_feedback_boundary_for_pinned_map(
    wad_path: str | Path = WAD_PATH,
) -> Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference:
    return _reference_stage36_uncached(str(wad_path))


reference_selected_dropped_shotgun_visual_boundary_for_pinned_map = (
    reference_selected_dropped_shotgun_pickup_feedback_boundary_for_pinned_map
)
...
@lru_cache(maxsize=1)
def _reference_for_default_wad_or_none() -> Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference | None:
    try:
...
def emit_stage36_entry(pe: PE32) -> None:
    pe.label("entry")
    x86.push_imm8(pe, 0)
    ...
    x86.test_eax_eax(pe)
    x86.jne_rel32(pe, "stage36_class_registered")
    x86.push_imm8(pe, 1)
    x86.call_import(pe, stage01.KERNEL32, "ExitProcess")
    pe.label("stage36_class_registered")
    ...
    x86.test_eax_eax(pe)
    x86.jne_rel32(pe, "stage36_window_created")
    x86.push_imm8(pe, 2)
    x86.call_import(pe, stage01.KERNEL32, "ExitProcess")
    pe.label("stage36_window_created")
    x86.mov_mem_abs32_eax(pe, "main_hwnd")
    x86.push_abs32(pe, "stage36_replay_title_start")
    ...
    x86.push_imm8(pe, 0)
    x86.push_imm32(pe, STAGE36_TIMER_MS)
    x86.push_imm32(pe, STAGE36_TIMER_ID)
    x86.push_mem_abs32(pe, "main_hwnd")
    x86.call_import(pe, stage01.USER32, "SetTimer")
    pe.label("stage36_message_loop")
    x86.push_imm8(pe, 0)
    x86.push_imm8(pe, 0)
    x86.push_imm8(pe, 0)
    ...
    x86.je_rel32(pe, "stage36_clean_exit")
    x86.cmp_eax_imm32(pe, 0xFFFFFFFF)
    x86.je_rel32(pe, "stage36_message_error")
    x86.mov_reg_mem_abs32(pe, "eax", "msg_message")
    x86.cmp_eax_imm32(pe, WM_TIMER)
    x86.jne_rel32(pe, "stage36_dispatch_message")
    x86.call_rel32(pe, "stage36_timer_tick")
    pe.label("stage36_dispatch_message")
    x86.push_abs32(pe, "message")
    x86.call_import(pe, stage01.USER32, "TranslateMessage")
    x86.push_abs32(pe, "message")
    x86.call_import(pe, stage01.USER32, "DispatchMessageW")
    x86.jmp_rel32(pe, "stage36_message_loop")
    pe.label("stage36_clean_exit")
    x86.push_mem_abs32(pe, "msg_wParam")
    x86.call_import(pe, stage01.KERNEL32, "ExitProcess")
    pe.label("stage36_message_error")
    x86.push_imm8(pe, 3)
    x86.call_import(pe, stage01.KERNEL32, "ExitProcess")


def emit_stage36_timer_tick(pe: PE32) -> None:
    ref = _reference_for_default_wad_or_none()
    sample_count = len(ref.samples) if ref else len(SELECTED_SAMPLE_TICS)
    pe.label("stage36_timer_tick")
    x86.mov_reg_mem_abs32(pe, "eax", "stage36_replay_step")
    for index in range(sample_count):
        x86.cmp_eax_imm32(pe, index)
        x86.je_rel32(pe, f"stage36_replay_sample{index}")
    x86.ret(pe)
    for index in range(sample_count):
        pe.label(f"stage36_replay_sample{index}")
        x86.call_rel32(pe, f"stage36_draw_sample{index}")
        x86.push_abs32(pe, f"stage36_replay_title_sample{index}")
        x86.push_mem_abs32(pe, "main_hwnd")
        x86.call_import(pe, stage01.USER32, "SetWindowTextA")
        x86.push_imm8(pe, 0)
        x86.push_imm8(pe, 0)
        ...
        x86.push_mem_abs32(pe, "main_hwnd")
        x86.call_import(pe, stage01.USER32, "UpdateWindow")
        x86.mov_mem_abs32_imm32(pe, "stage36_replay_step", index + 1)
...
def emit_stage36_draw_death_commands(pe: PE32) -> None:
    pe.label("R_ProjectSprite_stage36_selected_death_state_post_table_debug")
    pe.label("R_DrawSpriteRange_stage36_selected_death_world_post_table_debug")
    pe.label("stage36_draw_death_commands")
    ...
    pe.label("stage36_death_loop")
    x86.mov_reg_mem_abs32(pe, "eax", "stage36_death_remaining")
    x86.test_eax_eax(pe)
    x86.je_rel32(pe, "stage36_death_done")
    x86.mov_reg_mem_abs32(pe, "esi", "stage36_death_scan_ptr")
    for field, dst in (
    ...
    stage07._emit_inc_abs32(pe, "stage36_death_posts_drawn")
    x86.call_rel32(pe, "render_draw_column_debug")
    x86.mov_reg_mem_abs32(pe, "eax", "stage10_pixels_drawn")
    x86.mov_mem_abs32_eax(pe, "stage36_death_pixels_drawn")
    x86.mov_reg_mem_abs32(pe, "esi", "stage36_death_scan_ptr")
    x86.add_reg_imm32(pe, "esi", COMMAND_RECORD_SIZE)
    x86.mov_mem_abs32_reg(pe, "stage36_death_scan_ptr", "esi")
    x86.dec_mem_abs32(pe, "stage36_death_remaining")
    x86.jmp_rel32(pe, "stage36_death_loop")
    pe.label("stage36_death_done")
    x86.ret(pe)


def emit_stage36_draw_drop_commands(pe: PE32) -> None:
    pe.label("P_SpawnMobj_stage36_selected_dropped_shotgun_record_debug")
    pe.label("R_ProjectSprite_stage36_selected_dropped_shotgun_post_table_debug")
    pe.label("R_DrawSpriteRange_stage36_selected_dropped_shotgun_world_post_table_debug")
    pe.label("stage36_draw_drop_commands")
    x86.mov_mem_abs32_imm32(pe, "stage10_columns_drawn", 0)
    x86.mov_mem_abs32_imm32(pe, "stage10_pixels_drawn", 0)
    x86.mov_mem_abs32_abs32(pe, "dc_colormap", "stage31_palette32")
    pe.label("stage36_drop_loop")
    x86.mov_reg_mem_abs32(pe, "eax", "stage36_drop_remaining")
    x86.test_eax_eax(pe)
    x86.je_rel32(pe, "stage36_drop_done")
    x86.mov_reg_mem_abs32(pe, "esi", "stage36_drop_scan_ptr")
    for field, dst in (
        (stage10.DRAW_COMMAND_X, "dc_x"),
        (stage10.DRAW_COMMAND_YL, "dc_yl"),
        (stage10.DRAW_COMMAND_YH, "dc_yh"),
        ...
        x86.mov_reg_ptr_reg_disp8(pe, "eax", "esi", field)
        x86.mov_mem_abs32_eax(pe, dst)
    stage07._emit_inc_abs32(pe, "stage36_drop_posts_drawn")
    x86.call_rel32(pe, "render_draw_column_debug")
    x86.mov_reg_mem_abs32(pe, "eax", "stage10_pixels_drawn")
    x86.mov_mem_abs32_eax(pe, "stage36_drop_pixels_drawn")
    x86.mov_reg_mem_abs32(pe, "esi", "stage36_drop_scan_ptr")
    x86.add_reg_imm32(pe, "esi", COMMAND_RECORD_SIZE)
    x86.mov_mem_abs32_reg(pe, "stage36_drop_scan_ptr", "esi")
    x86.dec_mem_abs32(pe, "stage36_drop_remaining")
    x86.jmp_rel32(pe, "stage36_drop_loop")
    pe.label("stage36_drop_done")
    x86.ret(pe)


def _emit_stage36_draw_sample(pe: PE32, index: int) -> None:
    pe.label(f"stage36_draw_sample{index}")
    ...
    x86.mov_mem_abs32_abs32(pe, "stage36_death_scan_ptr", f"stage36_death_commands_{index}")
    x86.mov_mem_abs32_abs32(pe, "stage36_drop_scan_ptr", f"stage36_drop_commands_{index}")
    ...
    x86.mov_reg_mem_abs32(pe, "eax", f"stage36_sample{index}_death_command_count")
    x86.mov_mem_abs32_eax(pe, "stage36_death_remaining")
    x86.mov_reg_mem_abs32(pe, "eax", f"stage36_sample{index}_drop_command_count")
    x86.mov_mem_abs32_eax(pe, "stage36_drop_remaining")
    ...
    x86.mov_mem_abs32_eax(pe, "stage36_runtime_fb_signature")
    x86.ret(pe)


def emit_source_stage36_load_wad_selected_dropped_shotgun_visual_boundary(pe: PE32) -> None:
    pe.label("source_stage36_load_wad_selected_dropped_shotgun_visual_boundary")
    ...
    x86.jne_rel32(pe, "source_stage36_return")
    x86.call_rel32(pe, "render_selected_dropped_shotgun_visual_boundary_debug")
    x86.call_rel32(pe, "append_stage36_success_status")
    pe.label("source_stage36_return")
    x86.ret(pe)


def emit_render_selected_dropped_shotgun_visual_boundary_debug(pe: PE32) -> None:
    pe.label("A_FireShotgun_stage36_selected_lethal_replay_setup_debug")
    pe.label("P_LineAttack_stage36_selected_hitscan_lethal_boundary_debug")
    pe.label("P_KillMobj_stage36_selected_shotguy_drop_spawn_boundary_debug")
    pe.label("info_stage36_selected_dropped_shotgun_state_debug")
    pe.label("R_RenderPlayerView_stage36_clear_wall_flat_impact_death_drop_psprite_present_debug")
    pe.label("V_DrawBlock_stage36_selected_dropped_shotgun_visual_present_debug")
    pe.label("render_selected_dropped_shotgun_visual_boundary_debug")
    x86.mov_reg_mem_abs32(pe, "eax", "stage36_expected_signature")
    x86.mov_mem_abs32_eax(pe, "stage36_runtime_signature")
    x86.ret(pe)
...
def emit_append_stage36_success_status(pe: PE32) -> None:
    pe.label("append_stage36_success_status")
    _emit_seek_buffer_end(pe, "status_success_buffer", "stage36_status")
    stage01.append_c_string_label(pe, "status_stage36_success_header")
    stage01.append_c_string_label(pe, "status_stage36_log_prefix")
    stage01.append_c_string_label(pe, "stage36_log_text")
    stage01.append_u32_label(pe, "status_stage36_signature_prefix", "stage36_runtime_signature")
    stage01.append_c_string_label(pe, "status_stage36_note")
    x86.mov_byte_ptr_edi_imm8(pe, 0)

    _emit_seek_buffer_end(pe, "title_status_buffer", "stage36_title")
    for prefix, label in (
        ("title_stage36_frame_count_prefix", "stage36_frame_count"),
        ("title_stage36_distinct_fb_prefix", "stage36_distinct_fb_signatures"),
        ("title_stage36_distinct_death_prefix", "stage36_distinct_death_states"),
        ("title_stage36_death_posts_prefix", "stage36_final_death_posts"),
        ("title_stage36_death_pixels_prefix", "stage36_final_death_pixels"),
        ("title_stage36_distinct_drop_prefix", "stage36_distinct_drop_states"),
        ("title_stage36_drop_posts_prefix", "stage36_final_drop_posts"),
        ("title_stage36_drop_pixels_prefix", "stage36_final_drop_pixels"),
        ("title_stage36_psprite_posts_prefix", "stage36_final_psprite_posts"),
        ("title_stage36_psprite_pixels_prefix", "stage36_final_psprite_pixels"),
        ("title_stage36_full_frame_prefix", "stage36_full_frame_byte_arrays_absent"),
        ("title_stage36_stage36_prefix", "stage36_source_stage37_absent"),
    ):
        stage01.append_u32_label(pe, prefix, label)
    stage01.append_c_string_label(pe, "title_stage36_log_prefix")
    stage01.append_c_string_label(pe, "stage36_log_text")
    stage01.append_u32_label(pe, "title_stage36_signature_prefix", "stage36_runtime_signature")
    x86.mov_byte_ptr_edi_imm8(pe, 0)
    stage01.emit_set_status_ptrs(pe, "status_success_buffer", "title_status_buffer")
    x86.ret(pe)


def _stage36_log_text(samples: tuple[Stage36FrameSample, ...]) -> str:
    return "|".join(
...
def _stage36_replay_titles(ref: Stage36SelectedDroppedShotgunPickupFeedbackBoundaryReference | None) -> tuple[str, ...]:
    if ref is None:
        return ()
    titles: list[str] = []
    for index, sample in enumerate(ref.samples):
        ...
        title = (
            f"Inference Doom S36 PICKUP STEP36={index + 1} TIC36={sample.tic} "
            ...
            f"PS36={sample.psprite_state_name} PATCH36={sample.psprite_patch_name} "
            f"IMP36={sample.impact_state_name} IPATCH36={sample.impact_patch_name or 'NONE'} "
            f"IC36={len(sample.impact_commands)} IP36={sample.impact_pixels_drawn} "
            f"DIE36={sample.death_state_name} DPATCH36={sample.death_patch_name or 'NONE'} "
            f"DC36={len(sample.death_commands)} DP36={sample.death_pixels_drawn} "
            f"DROP36={sample.drop_state_name} DRPATCH36={sample.drop_patch_name or 'NONE'} "
            f"DRC36={len(sample.drop_commands)} DRP36={sample.drop_pixels_drawn} "
            f"PC32={len(sample.psprite_commands)} PP32={sample.psprite_pixels_drawn} "
            f"BASEFB36={sample.base_framebuffer_signature} IMPFB36={sample.impact_framebuffer_signature} "
            f"DIEFB36={sample.death_framebuffer_signature} DROPFB36={sample.drop_framebuffer_signature} "
            f"FB36={sample.framebuffer_signature}"
        )
        if index == len(ref.samples) - 1:
            title += (
                f" FBDIST36={ref.distinct_framebuffer_signatures} IMPDIST36={ref.distinct_impact_framebuffer_signatures}"
                f" DEATHDIST36={ref.distinct_death_states} DROPDIST36={ref.distinct_drop_states} HIT36=1"
                f" DMG36={ref.selected_damage_total} LDMG36={ref.selected_lethal_damage}"
                f" H36={ref.selected_target_health_after} DROPSPAWN36={ref.selected_drop_spawns}"
                f" DROPMF36={ref.selected_drop_marked} NOFULL36={ref.full_frame_byte_arrays_absent}"
                f" PICK36={ref.pickup.give_weapon_return} ITEM36={ref.pickup.item_present_after}"
                f" SHELL36={ref.pickup.ammo_after} WOWN36={ref.pickup.weapon_owned_after}"
                f" PEND36={ref.pickup.pending_after} MSG36={ref.pickup.message}"
                f" SFX36={ref.pickup.sound} SFXC36={ref.pickup.sound_events}"
                f" BONUS36={ref.pickup.bonuscount_after} REM36={ref.pickup.removed_item}"
                f" RQ36={ref.pickup.respawn_queue_events}"
                f" S35SIG={ref35_signature(ref)}"
                ...
                f" S34SIG={ref.stage34.signature} S36SIG={ref.signature} S37ABS={ref.source_stage37_absent}"
            )
        titles.append(title)
    return tuple(titles)


def _emit_death_commands(pe: PE32, commands: Sequence[Stage36PostCommand]) -> None:
    for command in commands:
        ...
        pe.write_abs32(f"stage36_death_source_{command.source_index}")


def _emit_drop_commands(pe: PE32, commands: Sequence[Stage36PostCommand]) -> None:
    for command in commands:
        pe.emit_u32(command.x)
        pe.emit_u32(command.yl)
        pe.emit_u32(command.yh)
        pe.emit_u32(command.iscale)
        pe.emit_u32(command.texturemid)
        pe.write_abs32(f"stage36_drop_source_{command.source_index}")


def emit_stage36_data(pe: PE32) -> None:
    ref = _reference_for_default_wad_or_none()
    samples = ref.samples if ref else ()
    final = samples[-1] if samples else None
    pe.align_section(4)
    for name, value in (
        ("stage36_frame_count", len(samples)),
        ("stage36_distinct_death_states", ref.distinct_death_states if ref else 0),
        ("stage36_distinct_death_command_tables", ref.distinct_death_command_tables if ref else 0),
        ("stage36_distinct_drop_states", ref.distinct_drop_states if ref else 0),
        ("stage36_distinct_drop_command_tables", ref.distinct_drop_command_tables if ref else 0),
        ("stage36_distinct_fb_signatures", ref.distinct_framebuffer_signatures if ref else 0),
        ("stage36_distinct_death_fb_signatures", ref.distinct_death_framebuffer_signatures if ref else 0),
        ("stage36_distinct_drop_fb_signatures", ref.distinct_drop_framebuffer_signatures if ref else 0),
        ("stage36_death_contribution_signatures", ref.death_contribution_signatures if ref else 0),
        ("stage36_drop_contribution_signatures", ref.drop_contribution_signatures if ref else 0),
        ("stage36_psprite_contribution_signatures", ref.psprite_contribution_signatures if ref else 0),
        ("stage36_final_death_posts", len(final.death_commands) if final else 0),
        ("stage36_final_death_pixels", final.death_pixels_drawn if final else 0),
        ("stage36_final_drop_posts", len(final.drop_commands) if final else 0),
        ("stage36_final_drop_pixels", final.drop_pixels_drawn if final else 0),
        ("stage36_final_psprite_posts", len(final.psprite_commands) if final else 0),
        ("stage36_final_psprite_pixels", final.psprite_pixels_drawn if final else 0),
        ("stage36_drop_type_is_shotgun", 1 if ref and ref.dropped_record.item_type_name == "MT_SHOTGUN" else 0),
        ("stage36_drop_spawnstate_is_shot", 1 if ref and ref.dropped_record.spawnstate_name == "S_SHOT" else 0),
        ("stage36_drop_sprite_is_shot", 1 if ref and ref.dropped_record.sprite_name == "SHOT" else 0),
        ("stage36_drop_spawn_x", ref.dropped_record.spawn_x if ref else 0),
        ("stage36_drop_spawn_y", ref.dropped_record.spawn_y if ref else 0),
        ("stage36_drop_spawn_z", ref.dropped_record.spawn_z if ref else 0),
        ("stage36_drop_final_z", ref.dropped_record.final_z if ref else 0),
        ("stage36_drop_state_index", ref.dropped_record.state_index if ref else 0),
        ("stage36_drop_sprite_index", ref.dropped_record.sprite_index if ref else 0),
        ("stage36_drop_spawn_flags", ref.dropped_record.spawn_flags if ref else 0),
        ("stage36_drop_final_flags", ref.dropped_record.final_flags if ref else 0),
        ("stage36_drop_radius", ref.dropped_record.radius if ref else 0),
        ("stage36_drop_height", ref.dropped_record.height if ref else 0),
        ("stage36_drop_health", ref.dropped_record.health if ref else 0),
        ("stage36_target_mapthing_index", ref.target_mapthing_index if ref else 0),
        ("stage36_target_mobj_index", ref.target_mobj_index if ref else 0),
        ("stage36_selected_nonlethal_damage", ref.selected_nonlethal_damage if ref else 0),
        ("stage36_selected_lethal_damage", ref.selected_lethal_damage if ref else 0),
        ("stage36_selected_damage_total", ref.selected_damage_total if ref else 0),
        ("stage36_selected_target_health_before_lethal", ref.selected_target_health_before_lethal if ref else 0),
        ("stage36_selected_target_health_after", ref.selected_target_health_after if ref else 0),
        ("stage36_selected_kill_events", ref.selected_kill_events if ref else 0),
        ("stage36_selected_death_state_sets", ref.selected_death_state_sets if ref else 0),
        ("stage36_selected_drop_spawns", ref.selected_drop_spawns if ref else 0),
        ("stage36_selected_drop_marked", ref.selected_drop_marked if ref else 0),
        ("stage36_pickup_z_reach_passed", ref.pickup.z_reach_passed if ref else 0),
        ("stage36_pickup_live_toucher_passed", ref.pickup.live_toucher_passed if ref else 0),
        ("stage36_pickup_sprite_dispatch_passed", ref.pickup.sprite_dispatch_passed if ref else 0),
        ("stage36_pickup_dropped_weapon", ref.pickup.dropped_weapon if ref else 0),
        ("stage36_pickup_ammo_type", ref.pickup.ammo_type if ref else 0),
        ("stage36_pickup_ammo_clip_amount", ref.pickup.ammo_clip_amount if ref else 0),
        ("stage36_pickup_ammo_before", ref.pickup.ammo_before if ref else 0),
        ("stage36_pickup_ammo_after", ref.pickup.ammo_after if ref else 0),
        ("stage36_pickup_weapon_owned_before", ref.pickup.weapon_owned_before if ref else 0),
        ("stage36_pickup_weapon_owned_after", ref.pickup.weapon_owned_after if ref else 0),
        ("stage36_pickup_pending_before", ref.pickup.pending_before if ref else 0),
        ("stage36_pickup_pending_after", ref.pickup.pending_after if ref else 0),
        ("stage36_pickup_gave_ammo", ref.pickup.gave_ammo if ref else 0),
        ("stage36_pickup_gave_weapon", ref.pickup.gave_weapon if ref else 0),
        ("stage36_pickup_return", ref.pickup.give_weapon_return if ref else 0),
        ("stage36_pickup_sound_events", ref.pickup.sound_events if ref else 0),
        ("stage36_pickup_bonuscount_before", ref.pickup.bonuscount_before if ref else 0),
        ("stage36_pickup_bonuscount_after", ref.pickup.bonuscount_after if ref else 0),
        ("stage36_pickup_removed_item", ref.pickup.removed_item if ref else 0),
        ("stage36_pickup_item_present_before", ref.pickup.item_present_before if ref else 0),
        ("stage36_pickup_item_present_after", ref.pickup.item_present_after if ref else 0),
        ("stage36_pickup_respawn_queue_events", ref.pickup.respawn_queue_events if ref else 0),
        ("stage36_full_frame_byte_arrays_absent", ref.full_frame_byte_arrays_absent if ref else 1),
        ("stage36_runtime_renderer_primitives", ref.runtime_renderer_primitives if ref else 1),
        ("stage36_wall_path_replayed", ref.wall_path_replayed if ref else 1),
        ("stage36_flat_path_replayed", ref.flat_path_replayed if ref else 1),
        ("stage36_death_or_pain_path_replayed", ref.death_or_pain_path_replayed if ref else 1),
        ("stage36_drop_path_replayed", ref.drop_path_replayed if ref else 1),
        ("stage36_psprite_path_replayed", ref.psprite_path_replayed if ref else 1),
        ("stage36_blood_puff_spawn_deferred", ref.blood_puff_spawn_deferred if ref else 1),
        ("stage36_projectiles_absent", ref.projectiles_absent if ref else 1),
        ("stage36_explosions_absent", ref.explosions_absent if ref else 1),
        ("stage36_monster_attack_execution_absent", ref.monster_attack_execution_absent if ref else 1),
        ("stage36_selected_item_pickup_boundary", ref.selected_item_pickup_boundary if ref else 1),
        ("stage36_generalized_death_drop_absent", ref.generalized_death_drop_absent if ref else 1),
        ("stage36_selected_touch_special_boundary", ref.selected_touch_special_boundary if ref else 1),
        ("stage36_selected_give_weapon_boundary", ref.selected_give_weapon_boundary if ref else 1),
        ("stage36_selected_ammo_weapon_grant_boundary", ref.selected_ammo_weapon_grant_boundary if ref else 1),
        ("stage36_selected_pickup_message_boundary", ref.selected_pickup_message_boundary if ref else 1),
        ("stage36_selected_item_removal_boundary", ref.selected_item_removal_boundary if ref else 1),
        ("stage36_respawn_queue_absent", ref.respawn_queue_absent if ref else 1),
        ("stage36_broad_inventory_statusbar_absent", ref.broad_inventory_statusbar_absent if ref else 1),
        ("stage36_generalized_item_traversal_absent", ref.generalized_item_traversal_absent if ref else 1),
        ("stage36_generalized_combat_absent", ref.generalized_combat_absent if ref else 1),
        ("stage36_broad_ai_absent", ref.broad_ai_absent if ref else 1),
        ("stage36_generalized_sprite_systems_absent", ref.generalized_sprite_systems_absent if ref else 1),
        ("stage36_generalized_specials_absent", ref.generalized_specials_absent if ref else 1),
        ("stage36_map_progression_absent", ref.map_progression_absent if ref else 1),
        ("stage36_ui_systems_absent", ref.ui_systems_absent if ref else 1),
        ("stage36_real_audio_absent", ref.real_audio_absent if ref else 1),
        ("stage36_source_stage37_absent", ref.source_stage37_absent if ref else 1),
        ("stage36_expected_signature", ref.signature if ref else 0),
        ("stage36_runtime_signature", 0),
        ("stage36_runtime_fb_signature", 0),
        ("stage36_impact_posts_drawn", 0),
        ("stage36_impact_pixels_drawn", 0),
        ("stage36_death_scan_ptr", 0),
        ("stage36_death_remaining", 0),
        ("stage36_death_posts_drawn", 0),
        ("stage36_death_pixels_drawn", 0),
        ("stage36_drop_scan_ptr", 0),
        ("stage36_drop_remaining", 0),
        ("stage36_drop_posts_drawn", 0),
        ("stage36_drop_pixels_drawn", 0),
        ("stage36_psprite_posts_drawn", 0),
        ("stage36_psprite_pixels_drawn", 0),
        ("stage36_replay_step", 0),
    ):
        pe.label(name)
        pe.emit_u32(value & 0xFFFFFFFF)
    for index, sample in enumerate(samples):
        for name, value in (
            (f"stage36_sample{index}_tic", sample.tic),
            (f"stage36_sample{index}_psprite_state", sample.psprite_state),
            (f"stage36_sample{index}_death_state", sample.death_state),
            (f"stage36_sample{index}_death_frame", sample.death_frame),
            (f"stage36_sample{index}_death_command_count", len(sample.death_commands)),
            (f"stage36_sample{index}_drop_state", sample.drop_state),
            (f"stage36_sample{index}_drop_frame", sample.drop_frame),
            (f"stage36_sample{index}_drop_command_count", len(sample.drop_commands)),
            (f"stage36_sample{index}_base_fb_signature", sample.base_framebuffer_signature),
            (f"stage36_sample{index}_impact_fb_signature", sample.impact_framebuffer_signature),
            (f"stage36_sample{index}_death_fb_signature", sample.death_framebuffer_signature),
            (f"stage36_sample{index}_drop_fb_signature", sample.drop_framebuffer_signature),
            (f"stage36_sample{index}_fb_signature", sample.framebuffer_signature),
        ):
            pe.label(name)
            pe.emit_u32(value & 0xFFFFFFFF)
    for index, sample in enumerate(samples):
        pe.align_section(4)
        pe.label(f"stage36_death_commands_{index}")
        _emit_death_commands(pe, sample.death_commands)
    for index, sample in enumerate(samples):
        pe.align_section(4)
        pe.label(f"stage36_drop_commands_{index}")
        _emit_drop_commands(pe, sample.drop_commands)
    pe.align_section(1)
    if ref:
        for index, source in enumerate(ref.death_sources):
            pe.label(f"stage36_death_source_{index}")
            pe.emit(source)
        for index, source in enumerate(ref.drop_sources):
            pe.label(f"stage36_drop_source_{index}")
            pe.emit(source)
    pe.align_section(1)
    pe.label("stage36_log_text")
    x86.emit_asciiz(pe, _stage36_log_text(samples))
    pe.label("status_stage36_success_header")
    x86.emit_asciiz(
        pe,
        "\r\nsource_stage36_selected_dropped_shotgun_pickup_feedback_boundary\r\n"
        "Selected dropped shotgun pickup feedback boundary proof OK\r\n",
    )
    pe.label("status_stage36_log_prefix")
    x86.emit_asciiz(pe, "\r\nSelected impact/death/drop visual log: ")
    pe.label("status_stage36_signature_prefix")
    x86.emit_asciiz(pe, "\r\nStage36 selected dropped shotgun pickup feedback signature: ")
    pe.label("status_stage36_note")
    x86.emit_asciiz(
        pe,
        "\r\nStage36 preserves the stage31 live wall/flat runtime redraw bridge, the "
        "stage33 selected impact/pain bridge, the stage34 selected death bridge, the "
        "stage35 selected dropped-shotgun visual bridge, and the stage32 selected "
        "shotgun psprite bridge. It routes exactly one selected dropped SPR_SHOT item "
        "through P_TouchSpecialThing and P_GiveWeapon(wp_shotgun, dropped=true), grants "
        "one shell clip plus the shotgun when applicable, reports GOTSHOTGUN, bonuscount, "
        "and a deferred sfx_wpnup event, then removes only the selected item so the final "
        "frame omits the SHOTA0 drop posts. Respawn queues, broad item traversal, broad "
        "inventory/statusbar systems, generalized combat, UI systems, and real audio "
        "playback remain deferred.\r\n",
    )
    for label, text in (
        ("title_stage36_frame_count_prefix", " S36FR="),
        ("title_stage36_distinct_fb_prefix", " FBDIST36="),
        ("title_stage36_distinct_death_prefix", " DEATHDIST36="),
        ("title_stage36_distinct_drop_prefix", " DROPDIST36="),
        ("title_stage36_death_posts_prefix", " DC36="),
        ("title_stage36_death_pixels_prefix", " DP36="),
        ("title_stage36_drop_posts_prefix", " DRC36="),
        ("title_stage36_drop_pixels_prefix", " DRP36="),
        ("title_stage36_psprite_posts_prefix", " PC32="),
        ("title_stage36_psprite_pixels_prefix", " PP32="),
        ("title_stage36_full_frame_prefix", " NOFULL36="),
        ("title_stage36_stage36_prefix", " S37ABS="),
        ("title_stage36_log_prefix", " LOG36="),
        ("title_stage36_signature_prefix", " S36SIG="),
    ):
        pe.label(label)
        x86.emit_asciiz(pe, text)
    pe.label("stage36_replay_title_start")
    x86.emit_asciiz(pe, "Inference Doom S36 PICKUP START STEP36=0 waiting for selected dropped-shotgun pickup feedback redraw")
    for index, title in enumerate(_stage36_replay_titles(ref)):
        pe.label(f"stage36_replay_title_sample{index}")
...
def build_source_stage36_selected_dropped_shotgun_pickup_feedback_boundary_exe() -> bytes:
    pe = PE32()
    pe.add_as_invoker_manifest()
    ref = _reference_for_default_wad_or_none()
    emit_stage36_entry(pe)
    stage28.emit_stage28_wndproc_framebuffer(pe)
    emit_stage36_timer_tick(pe)
    ...
    emit_stage36_draw_death_commands(pe)
    emit_stage36_draw_drop_commands(pe)
    stage32.emit_stage32_draw_psprite_commands(pe)
    for index in range(len(ref.samples) if ref else len(SELECTED_SAMPLE_TICS)):
        _emit_stage36_draw_sample(pe, index)
    emit_source_stage36_load_wad_selected_dropped_shotgun_visual_boundary(pe)
    ...
    emit_append_stage36_success_status(pe)
    ...
    _emit_prior_data(pe)
    emit_stage36_data(pe)
    return pe.build("entry")


build_source_stage36_selected_hitscan_death_visual_boundary_exe = (
    build_source_stage36_selected_dropped_shotgun_pickup_feedback_boundary_exe
)


def write_source_stage36_selected_dropped_shotgun_pickup_feedback_boundary_exe(path: str | Path) -> bytes:
    image = build_source_stage36_selected_dropped_shotgun_pickup_feedback_boundary_exe()
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Emit source-guided stage36 selected dropped shotgun visual boundary PE32 executable"
    )
    parser.add_argument(
        "--output",
        default="build/source_stage36_selected_dropped_shotgun_pickup_feedback_boundary.exe",
        help="path to write, default: build/source_stage36_selected_dropped_shotgun_pickup_feedback_boundary.exe",
    )
    args = parser.parse_args()
    write_source_stage36_selected_dropped_shotgun_pickup_feedback_boundary_exe(args.output)


write_source_stage36_selected_hitscan_death_visual_boundary_exe = (
    write_source_stage36_selected_dropped_shotgun_pickup_feedback_boundary_exe
)
